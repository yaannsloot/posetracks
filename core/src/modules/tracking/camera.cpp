...
Rt solveStaticPair(const TrackedPoints& t_points_a, const TrackedPoints& t_points_b, const Kk& cam_Kk_a, const Kk& cam_Kk_b)
{
	...
	cv::undistortPoints(t_points_a, undis_points_a, cam_Kk_a.K, cam_Kk_a.dist_vector());
	cv::undistortPoints(t_points_b, undis_points_b, cam_Kk_b.K, cam_Kk_b.dist_vector());
...
std::vector<Rt> solveStaticSet(const std::vector<TrackingData>& t_data, const std::vector<Kk>& cam_Kk)
{
	...
	// SOLUTION FOR MORE THAN TWO VIEWS
...
Rt solveCameraWithTag(const Tag& observed_tag, const Kk& cam_Kk, const double square_length)
{
...
Mat4x4 calculateProcrustesTransform(const std::vector<Rt>& orig_cam_Rt, const std::vector<Tag>& observed_tags)
{
	/*
	* TODO
	* 
