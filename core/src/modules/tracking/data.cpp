...
namespace me::tracking {
	...
	void TrackingData::set_pose(const int frame, const std::string name, const dnn::Pose& pose)
	{
		this->poses[frame][name] = pose;
	}

	void TrackingData::set_joint(const int frame, const std::string name, const int id, const dnn::Joint& joint)
	{
	...
	{
		this->poses[frame][name][id] = dnn::Joint(pt, prob);
	}
	...
	{
		this->poses[frame][name][id] = dnn::Joint(x, y, prob);
	}

	void TrackingData::set_detection(const int frame, const std::string name, const dnn::Detection& det)
	{
		this->detections[frame][name] = det;
	}

	void TrackingData::set_tag(const int frame, const int id, const dnn::Tag& tag)
	{
